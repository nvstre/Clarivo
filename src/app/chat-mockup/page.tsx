  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f7fafd] overflow-x-hidden">
      <div className="flex flex-1 w-full max-w-full overflow-x-hidden">
        ...
        <aside className="hidden md:flex flex-col justify-between bg-white shadow-sm rounded-none md:rounded-r-3xl py-8 px-6 min-h-screen w-full max-w-[270px]">
          <div>
        ...
        {sidebarOpen && (
          <div className="fixed inset-0 z-40 bg-black/40 flex md:hidden" onClick={() => setSidebarOpen(false)}>
            <aside className="bg-white w-4/5 max-w-[320px] h-full flex flex-col justify-between shadow-2xl p-6 relative" onClick={e => e.stopPropagation()}>
              <button className="absolute top-4 right-4 text-[#5f749b]" onClick={() => setSidebarOpen(false)}>
        ...
        {/* Main Content */}
        <main className="flex-1 flex flex-col px-0 py-8 w-full max-w-full overflow-x-hidden">
          {/* Top Bar */}
          {/* Mobile Header: hamburger only */}
          <div className="flex md:hidden items-center justify-between px-4 py-4 mb-4 w-full">
            <button className="text-[#5f749b]" onClick={() => setSidebarOpen(true)}>
              <Menu className="w-7 h-7" />
            </button>
            <span className="font-extrabold text-lg tracking-tight text-[#23244a]">CLARIVO</span>
            <div className="w-7 h-7" /> {/* Spacer for symmetry */}
          </div>
          {/* Desktop Top Bar */}
          <div className="hidden md:flex items-center justify-end gap-4 px-12 mb-6">
          ...
          <div className="px-2 sm:px-12 flex flex-col gap-4 items-center w-full">
            <div className="bg-white rounded-2xl shadow-md p-4 sm:p-6 flex flex-col gap-2 w-full max-w-2xl h-full min-h-0">
              ...
              <form className="flex items-center gap-2 sm:gap-4 mt-2" onSubmit={handleSubmit}>
                <input
                  type="text"
                  placeholder="Type anything...."
                  className="flex-1 rounded-full bg-white px-4 sm:px-6 py-2 sm:py-4 text-sm border border-[#e3e8f0] shadow focus:border-[#7b5cff] outline-none transition"
                  value={input}
                  onChange={e => setInput(e.target.value)}
                  disabled={loading}
                />
                <button type="submit" className="px-4 sm:px-10 py-2 sm:py-4 rounded-full bg-gradient-to-br from-[#7b5cff] to-[#4f8cff] text-white font-bold shadow hover:from-[#4f8cff] hover:to-[#7b5cff] transition text-sm sm:text-base">
